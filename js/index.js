...
// Add keyboard visibility detection and auto-scroll functionality
// This ensures the cursor is always visible when keyboard appears
(function setupMobileInputSupport() {
    // All input elements that might receive focus and trigger keyboard
    const allInputs = [inputText, outputText, password];
    const viewportHandler = {
        previousHeight: window.visualViewport ? window.visualViewport.height : window.innerHeight,
        // Track which element currently has focus
        focusedElement: null,
        
        // Handle visual viewport changes (like keyboard appearing)
        handleViewportChange: function() {
            if (!window.visualViewport) return;
            
            const currentHeight = window.visualViewport.height;
            const heightDifference = this.previousHeight - currentHeight;
            
            // If height significantly decreased, keyboard likely appeared
            if (heightDifference > 150 && this.focusedElement) {
                // Calculate element's bottom position relative to viewport
                const rect = this.focusedElement.getBoundingClientRect();
                const elementBottom = rect.bottom;
                
                // Get cursor position for textarea
                let cursorBottom = elementBottom;
                if (this.focusedElement.tagName.toLowerCase() === 'textarea') {
                    try {
                        // Try to get more precise cursor position for textareas
                        const cursorPosition = this.focusedElement.selectionEnd;
                        const textUpToCursor = this.focusedElement.value.substring(0, cursorPosition);
                        const textLines = textUpToCursor.split('\n');
                        const lineCount = textLines.length;
                        
                        // Approximate cursor position (this is a rough estimate)
                        // Better calculation would require measuring text, which is complex
                        const lineHeight = 20; // Approximate line height
                        const topPadding = 12; // From CSS padding
                        const cursorY = topPadding + (lineCount - 1) * lineHeight;
                        cursorBottom = rect.top + cursorY;
                    } catch(e) {
                        // Fallback to element bottom if calculation fails
                        cursorBottom = elementBottom;
                    }
                }
                
                // Check if cursor might be hidden under keyboard
                if (cursorBottom > currentHeight) {
                    // Calculate how much we need to scroll
                    const scrollNeeded = cursorBottom - currentHeight + 20; // Add 20px buffer
                    
                    // Smooth scroll to make cursor visible
                    window.scrollBy({
                        top: scrollNeeded,
                        behavior: 'smooth'
                    });
    ...
    allInputs.forEach(input => {
        input.addEventListener('focus', function() {
            viewportHandler.focusedElement = this;
        });
        
        input.addEventListener('blur', function() {
            // Small delay to ensure we don't clear the focused element
            // before the viewport change handler runs
            setTimeout(() => {
                if (viewportHandler.focusedElement === this) {
                    viewportHandler.focusedElement = null;
                }
            }, 100);
        });
        
        // For textareas, also track cursor position changes
        if (input.tagName.toLowerCase() === 'textarea') {
            input.addEventListener('click', function() {
                if (viewportHandler.focusedElement === this) {
                    viewportHandler.handleViewportChange();
                }
            });
    ...
    if (window.visualViewport) {
        ...
        window.addEventListener('resize', () => {
            ...
            const wasResized = viewportHandler.previousHeight !== currentHeight;
            
            if (wasResized) {
                viewportHandler.previousHeight = currentHeight;
                viewportHandler.handleViewportChange();
            }
        });
    }
