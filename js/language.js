...
function setLanguage(lang) {
    ...
    // IMPORTANT: Since switchMode is in index.js, we need to call it
    // We can make switchMode global or pass a reference. For simplicity,
    // assume switchMode will be available globally when this runs.
    if (typeof switchMode === 'function') {
         // Call switchMode (defined in index.js) to update dynamic text like placeholders
         switchMode(window.isEncryptMode ? 'encrypt' : 'decrypt', false); // Update placeholders without saving state
    } else {
        console.warn("switchMode function not found. Placeholders might not update immediately.");
    }
}
